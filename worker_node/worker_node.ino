...
//  Sensors: LoRa, WiFi-STA, OLED, RFID, SW-420,
//           MPU6050, Pulse Sensor, DHT11, SOS Button, Buzzer
// ============================================================
...
// ─── Buttons & Output ─────────────────────────────────────────
#define SW420_PIN  36   // Shock sensor (input-only)
#define PULSE_PIN  39   // Pulse sensor analog (input-only)
...
const float TX_POWER    = -40.0;  // RSSI at 1m from anchor
const float PATH_LOSS_N =  3.5;   // 3.5 = underground rock tunnel
...
const float FREEFALL_THRESHOLD = 3.5;   // m/s² — freefall
const float IMPACT_THRESHOLD   = 18.0;  // m/s² — hard impact

// ─── State ────────────────────────────────────────────────────
String currentTag           = "None";
String currentConnectedZone = "Unknown";
float  bodyTemp = 0.0, bodyHumid = 0.0;
int    bpm = 0;
int    lastPulseRaw = 0;
unsigned long lastBeatTime = 0;

// ════════════════════════════════════════════════════════════
//  BPM — Peak Detection on raw analog pulse sensor
// ════════════════════════════════════════════════════════════
int readBPM() {
...
//  Phase 1: Free-fall (acceleration drops below gravity)
//  Phase 2: Hard impact (spike above impact threshold)
// ════════════════════════════════════════════════════════════
bool detectMPUFall() {
...
struct AnchorReading { String ssid; int rssi; float dist_m; };
...
void showStatus(String zone, bool sos, bool fall, bool shock) {
  ...
  display.setCursor(0, 23); display.print("Tag:  "); display.println(currentTag.substring(0, 8));
...
void checkRFID() {
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    currentTag = tag;
    Serial.println("Worker RFID: " + tag);
    buzzerAck();
    mfrc522.PICC_HaltA();
  }
}
...
void setup() {
  Serial.begin(115200);
  SPI.begin(); Wire.begin(); mfrc522.PCD_Init(); dht.begin();
...
void loop() {
  ...
  Serial.print("RFID Tag   : "); Serial.println(currentTag);
  ...
  Serial.print("Health Alrt: "); Serial.println(healthAlert ? "YES!" : "No");
